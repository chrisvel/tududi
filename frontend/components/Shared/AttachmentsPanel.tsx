...
// Upload card plus one card per file, with preview and delete. Used for
// tasks and projects; `api` says whose files these are.
const AttachmentsPanel: React.FC<AttachmentsPanelProps> = ({
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (attachments.length >= 20) {
            showErrorToast(
                t('attachments.limitReached', 'Maximum 20 attachments allowed')
            );
            if (fileInputRef.current) {
                fileInputRef.current.value = '';
            }
        ...
        setUploading(true);
        try {
            const newAttachment = await api.upload(file);
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await api.remove(attachmentToDelete.uid);
    ...
    const handleDownload = (attachment: FileAttachment) => {
        window.open(api.downloadUrl(attachment.uid), '_blank');
    };
                ...
                >
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-center">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 text-center mt-1">
                            {t('task.attachments.anyFormat', 'Any file type')}
                        </p>
                    </div>
                </div>

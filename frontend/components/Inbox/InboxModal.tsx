...
const InboxModal: React.FC<InboxModalProps> = ({
    ...
    const removeDueDateFromText = () => {
        ...
        const newText = currentText
            ...
            .replace(
                /\b(by|next)\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/gi,
                ''
            )
            .replace(/\s+/g, ' ')
            .trim();
    ...
    const shouldKeepSuggestedTags = (text: string): boolean => {
        ...
        return lastSuggestedTagsRef.current.some((tag) =>
            shouldKeepSpecificTag(tag, text)
        );
    };
    ...
    const getAllTags = (text: string): string[] => {
        ...
        if (
            lastSuggestedTagsRef.current.length > 0 &&
            shouldKeepSuggestedTags(text)
        ) {
        ...
        if (isUrlContent) {
            const hasBookmarkTag = allTagNames.some(
    ...
    const getContentWithCurrentTagsAndProjects = (text: string): string => {
        ...
        if (currentProjects.length > 0) {
            const projectString = currentProjects
                .map((project) =>
    ...
    const getSuggestion = (): {
        ...
        if (type === 'note') {
            ...
            const isCodeSnippet =
                analysisResult.suggested_reason === 'code_snippet_detected';
            let message: string;
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (
                    ...
                    Array.isArray(result.suggested_tags)
                ) {
    ...
    const cleanTextFromTagsAndProjects = (text: string): string => {
        ...
        while (i < tokens.length) {
            ...
            const isDueDateKeyword =
                tokens[i].match(/^(today|tomorrow)$/i) ||
                (tokens[i].match(/^(by|next)$/i) &&
                    i + 1 < tokens.length &&
                    tokens[i + 1].match(
        ...
        async (forceInbox = false) => {
            ...
            try {
                ...
                if (analysisResult?.suggested_type === 'task' && !forceInbox) {
                    ...
                    const textPriority =
                        analysisResult?.parsed_priority ||
                        parsePriority(contentToSubmit.trim());
                    const suggestedPriority =
                        analysisResult?.suggested_priority;
                    const finalPriority =
                        textPriority || suggestedPriority || 'medium';

                    // Get due date from analysis result or parse from text
                    const dueDate =
                        analysisResult?.suggested_due_date ||
                        getDueDate(contentToSubmit.trim());
                ...
                if (editMode && onEdit) {
                    ...
                    const contentWithCurrentTags =
                        getContentWithCurrentTagsAndProjects(
                            contentToSubmit.trim()
                        );

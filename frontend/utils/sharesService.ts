...
export async function listShares(resource_type: ShareGrantRequest['resource_type'], resource_uid: string): Promise<ListSharesResponseRow[]> {
...
export async function revokeShare(resource_type: ShareGrantRequest['resource_type'], resource_uid: string, target_user_id: number): Promise<void> {
  const res = await fetch('/api/shares', {
    method: 'DELETE',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json', Accept: 'application/json' },

...
async function makeAdminDirect(userId) {
  await Role.findOrCreate({ where: { user_id: userId }, defaults: { user_id: userId, is_admin: true } });
}
...
describe('Admin Users Management API', () => {
  ...
  describe('POST /api/admin/users', () => {
    ...
    it('should create an admin user when role is admin', async () => {
      ...
      const role = await Role.findOne({ where: { user_id: res.body.id } });
  ...
  describe('DELETE /api/admin/users/:id', () => {
    ...
    it('should allow deleting another admin when more than one admin exists', async () => {
      ...
      const secondRes = await adminAgent
        .post('/api/admin/users')
        .send({ email: 'secondadmin@example.com', password: 'password123', role: 'admin' });
      expect(secondRes.status).toBe(201);
      const secondId = secondRes.body.id;
      // delete the other admin
      const delRes = await adminAgent.delete(`/api/admin/users/${secondId}`);
      expect(delRes.status).toBe(204);
    });

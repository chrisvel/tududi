...
async function getAccess(userId, resourceType, resourceUid) {
  ...
  if (resourceType === 'project') {
    const proj = await Project.findOne({ where: { uid: resourceUid }, attributes: ['user_id'], raw: true });
    if (!proj) return ACCESS.NONE;
    if (proj.user_id === userId) return ACCESS.RW;
  } else if (resourceType === 'task') {
    const t = await Task.findOne({ where: { uid: resourceUid }, attributes: ['user_id'], raw: true });
    if (!t) return ACCESS.NONE;
    if (t.user_id === userId) return ACCESS.RW;
  } else if (resourceType === 'note') {
    const n = await Note.findOne({ where: { uid: resourceUid }, attributes: ['user_id'], raw: true });
  ...
  const perm = await Permission.findOne({
    where: { user_id: userId, resource_type: resourceType, resource_uid: resourceUid },
...
async function ownershipOrPermissionWhere(resourceType, userId) {
  ...
  return {
    [Op.or]: [
      { user_id: userId },
      sharedUids.length ? { uid: { [Op.in]: sharedUids } } : { uid: null },
    ],
  };
}

module.exports = { ACCESS, getAccess, ownershipOrPermissionWhere, getSharedUidsForUser };

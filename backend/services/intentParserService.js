...
class IntentParserService {
    constructor() {
        ...
        // Define intent training examples (these are used to match user queries)
        this.intentExamples = {
            list_tasks: [
                'show me my tasks',
                'list all tasks',
        ...
        this.initPromise = (async () => {
            try {
                ...
                for (const [intent, examples] of Object.entries(this.intentExamples)) {
                    const embeddings = await this.model.embed(examples);
                    this.intentEmbeddings[intent] = await embeddings.array();
                    embeddings.dispose();
                }
    ...
    async parse(message) {
        ...
        try {
            ...
            const messageEmbedding = await this.model.embed([text]);
            ...
            for (const [intent, exampleEmbeddings] of Object.entries(this.intentEmbeddings)) {
                for (const exampleVector of exampleEmbeddings) {
                    const similarity = this.cosineSimilarity(messageVector, exampleVector);
            ...
            const needsAI = this.needsOpenAI(bestIntent, bestScore, text);

            // Build structured query
            const query = this.buildStructuredQuery(bestIntent, entities);
    ...
    fallbackParse(message) {
        ...
        if (text.includes('task') && (text.includes('show') || text.includes('list'))) {
            intent = 'list_tasks';
        } else if (text.includes('project') && (text.includes('show') || text.includes('list'))) {
        ...
        const entities = this.extractEntities(text);
        const query = this.buildStructuredQuery(intent, entities);
    ...
    extractEntities(text) {
        ...
        // Detect priority
        for (const [priority, keywords] of Object.entries(this.entityPatterns.priority)) {
        ...
        // Detect time period
        for (const [period, keywords] of Object.entries(this.entityPatterns.timePeriod)) {
            for (const keyword of keywords) {
                if (text.includes(keyword)) {
        ...
        // Detect item type
        for (const [type, keywords] of Object.entries(this.entityPatterns.itemType)) {
            for (const keyword of keywords) {
                if (text.includes(keyword)) {
        ...
        if (text.includes('productive') || text.includes('productivity') || text.includes('make me')) {
            entities.metrics = this.productivityMetrics;
        } else {
     ...
     */
    buildStructuredQuery(intent, entities) {
        const query = {
            intent,
     ...
     */
    needsOpenAI(intent, confidence, text) {

...
async function applyPerms(tx, changes) {
  ...
  for (const u of upserts) {
    const where = { user_id: u.userId, resource_type: u.resourceType, resource_uid: u.resourceUid };
    const existing = await Permission.findOne({ where, transaction: tx, lock: tx.LOCK.UPDATE });
    if (existing) {
      ...
      await existing.update({
        ...
        source_action_id: u.sourceActionId || existing.source_action_id || null,
      }, { transaction: tx });
    } else {
  ...
  for (const d of deletes) {
    await Permission.destroy({ where: { user_id: d.userId, resource_type: d.resourceType, resource_uid: d.resourceUid }, transaction: tx });
  }
}

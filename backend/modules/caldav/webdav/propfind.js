const { parsePropfind, buildMultistatus, buildResponse, buildPropstat, buildHref } = require('./utils');
...
async function handlePropfind(req, res) {
  try {
    ...
    let propfindRequest;
    try {
      propfindRequest = await parsePropfind(req.rawBody || '<?xml version="1.0"?><D:propfind xmlns:D="DAV:"><D:allprop/></D:propfind>');
    } catch (error) {
    ...
    if (isTaskRequest) {
      ...
      const response = await buildTaskResponse(task, username, propfindRequest);
      responses.push(response);
    } else {
      const calendarResponse = await buildCalendarResponse(username, userId, propfindRequest);
      responses.push(calendarResponse);

      if (depth > 0) {
        const tasks = await taskRepository.findByUser(userId);
        for (const task of tasks) {
          const taskResponse = await buildTaskResponse(task, username, propfindRequest);
...
async function buildTaskResponse(task, username, propfindRequest) {
  ...
  try {
    ...
    const propstat = buildPropstat(errorProps, 'HTTP/1.1 500 Internal Server Error');
    return buildResponse(href, propstat);
  }
}

const { parseCalendarQuery, buildMultistatus, buildResponse, buildPropstat, buildHref, buildCalendarData } = require('./utils');
...
async function handleReport(req, res) {
  try {
    ...
    try {
      ...
      return res.status(400).json({ error: 'Invalid calendar-query request' });
    }
    ...
    if (queryRequest.filters.textMatch) {
      const { property, value, caseless } = queryRequest.filters.textMatch;
    ...
    for (const task of tasks) {
      const response = await buildCalendarQueryResponse(task, username, queryRequest);
      if (response) {
...
async function buildCalendarQueryResponse(task, username, queryRequest) {
  try {
    ...
    console.error(`Error building calendar-query response for ${task.uid}:`, error);
    return null;
  }
}

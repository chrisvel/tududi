...
function parseCalendarQuery(xmlString) {
  return new Promise((resolve, reject) => {
    xmlParser.parseString(xmlString, (err, result) => {
      ...
      try {
        const query = result['C:calendar-query'] || result['calendar-query'];
        const filter = query?.['C:filter'] || query?.filter;
        const compFilter = filter?.['C:comp-filter'] || filter?.['comp-filter'];
        ...
        if (compFilter) {
          const timeRange = compFilter['C:time-range'] || compFilter['time-range'];
          if (timeRange) {
          ...
          const propFilter = compFilter['C:prop-filter'] || compFilter['prop-filter'];
          if (propFilter) {
            const textMatch = propFilter['C:text-match'] || propFilter['text-match'];
            if (textMatch) {
              parsedQuery.filters.textMatch = {
                property: propFilter.name,
                value: typeof textMatch === 'string' ? textMatch : textMatch._,
                caseless: textMatch['collation'] === 'i;unicode-casefold'
              };
            }
          }
        }
        ...
        if (prop) {
          parsedQuery.props = Object.keys(prop).map(key => key.replace(/^[^:]+:/, ''));
        }
      ...
      try {
        ...
        if (prop) {
          const requestedProps = Object.keys(prop).map(key => {
            ...
            return { name: name || namespace, namespace: name ? namespace : 'D' };
          });

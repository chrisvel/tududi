...
const { handleGetTask, handlePutTask, handleDeleteTask } = require('./webdav/task-handlers');

const router = express.Router();

router.get('/.well-known/caldav', handleWellKnown);

router.options('/caldav/:username/tasks/', xmlParser, caldavAuth, handleOptions);
router.options('/caldav/:username/tasks/:uid', xmlParser, caldavAuth, handleOptions);
...
router.get('/caldav/:username/tasks/', xmlParser, caldavAuth, (req, res) => {
  res.status(207).send('<?xml version="1.0"?><D:multistatus xmlns:D="DAV:"/>');
});

router.get('/caldav/:username/tasks/:uid', xmlParser, caldavAuth, handleGetTask);
router.put('/caldav/:username/tasks/:uid', xmlParser, caldavAuth, handlePutTask);
router.delete('/caldav/:username/tasks/:uid', xmlParser, caldavAuth, handleDeleteTask);

...
import { useDataContext } from '../../contexts/DataContext'; // Import the useDataContext hook
...
const ProjectDetails: React.FC = () => {
  ...
  const { areas, createArea, updateArea, deleteArea } = useDataContext(); // Use the context to get areas and manage them
  const { projects, updateProject, deleteProject } = useDataContext(); // Get context for projects

  const [project, setProject] = useState<any>(null); // Flexible project type
  ...
  useEffect(() => {
    // Simulate loading the project based on id from the context (if needed)
  ...
  const handleTaskCreate = async (taskData: Partial<any>) => {
    // Simulated function for task creation
  };
  ...
  const handleSaveProject = async (updatedProject: any) => {
    try {
      await updateProject(updatedProject.id, updatedProject); // Call update from context
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      await deleteProject(project.id); // Delete project via context
      navigate('/projects');
    } catch (err) {
  ...
  return (
    <div className="flex justify-center px-4">
      <div className="w-full max-w-4xl">
        ...
        <NewTask onTaskCreate={(taskName: string) => handleTaskCreate({ name: taskName, status: 'not_started', project_id: project?.id })} />
        ...
        <ProjectModal
          ...
          areas={areas} // Using areas from context
        />

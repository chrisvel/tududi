...
**Files.** Paste, drop, or pick files with the paperclip button and they are saved with the item. Only an Inbox item or a task can hold files, so Note and Project are unavailable while files are in the box, and adding a file while one of them is chosen moves the box to Task (the status line says so). A pasted screenshot on its own is enough: its file name becomes the title. Pasted images are renamed from the browser's generic `image.png` to `Pasted image <date time>.png`. When a copy carries both text and a picture (spreadsheet cells, for example), the text is pasted and the picture is ignored. With One item per line, the files go on the first item. Each file must be under the server's upload limit, and one item holds up to 20. If a file fails to upload, the item is still saved and the failed file names are shown.

When an Inbox item with files becomes a task, its files move onto the task. Files stay with the item if it becomes a note or project. Deleting an Inbox item deletes its files; trashing keeps them so the item can be restored. Inbox files are visible only to their owner.
